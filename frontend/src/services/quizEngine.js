 ...
 */
export const generateDynamicHints = (answer) => {
    if (answer.length === 0) {
        return { hint2: '', hint3: '' };
    }
 ...
 */
export const getNextQuiz = (genre, difficulty, playedQuizIds) => {
    const availableQuizzes = allQuizzes.filter((quiz) => quiz.genre === genre &&
 ...
 */
export const getShuffledOptions = (quiz) => {
...
const normalize = (str) => toKatakana(str.trim().replace(/\s/g, '').replace(/[\u30FC\uFF70]/g, 'ー'));
export const checkAnswer = (quiz, userAnswer) => {
...
export const getAllAvailableQuizzesCount = (genre, difficulty) => {
    return allQuizzes.filter((quiz) => quiz.genre === genre && quiz.difficulty === difficulty).length;
};
// 全ジャンル、難易度を取得する
...
export const getAvailableDifficultiesForGenre = (genre) => {
