...
let recognizingChangeCallback = null;
...
export const GENRE_READING_MAP = {
    'どうぶつ': '動物',
    'こんちゅう': '昆虫',
    'しょくぶつ': '植物',
    'さかな': '魚類',
...
export const detectVoiceCommand = (transcript) => {
    ...
    if (t.includes('ヒント1') || t.includes('ヒントいち'))
        return 'hint1';
    if (t.includes('ヒント2') || t.includes('ヒントに'))
        return 'hint2';
    if (t.includes('ヒント3') || t.includes('ヒントさん'))
        return 'hint3';
    if (t.includes('降参') || t.includes('答え') || t.includes('わからない') || t.includes('わからん'))
        return 'surrender';
    if (t.includes('問題') || t.includes('もういちど'))
