...
let speechRate = parseFloat(localStorage.getItem(SPEECH_RATE_STORAGE_KEY) || String(DEFAULT_SPEECH_RATE));
let voices = [];
// 音声リストの準備
const loadVoices = () => {
...
export const unlockAudioContext = () => {
    ...
    if (window.speechSynthesis.speaking || utterance)
        return;
 ...
 * {漢字|よみ} 記法をよみだけに変換します。
 */
export const toReadableText = (text) => text.replace(/\{([^|]+)\|([^}]+)\}/g, '$2');
/**
