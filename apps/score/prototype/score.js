    ...
    for (let sys = 0; sys < nSystems; sys++) {
      ...
      for (let hand = 0; hand < 2; hand++) {
        ...
        for (let m = 0; m <= nBars; m++) {
          ...
          cs.sort((a, b) => a.x - b.x);
          // 小節の中での横位置（0〜1）。読めなかった休符のぶんの時間を
          // どこに入れるべきかを、あとで推し量るのに使う
          {
            const mStart = m === 0 ? (st.xMusic || st.xStart || 0) : myBars[m - 1];
            const mEndX = m < nBars ? myBars[m] : (st.xEnd || mStart + 1);
            const w = Math.max(1, mEndX - mStart);
            for (const c of cs) c.bx = Math.max(0, Math.min(1, (c.x - mStart) / w));
          }
          parts[hand].push(cs);
    ...
    for (let hand = 0; hand < 2; hand++) {
      ...
      for (let m = 0; m < nMeasures; m++) {
        ...
        if (measureQ && chords.length) {
          ...
          // 足りない分は横位置の空きに休符で埋める（下でまとめて出す）
        }
        // 足りない時間（読めなかった休符や長さの読み違いのぶん）を、
        // 末尾ではなく「横位置から見ていちばん空いている場所」に休符として入れる。
        // 末尾に決め打ちすると、休符のあとに鳴る音が全部前に詰まる
        // （実測で3拍目の和音が0.5拍目に鳴っていた）。
        let padAt = chords.length, padDur = 0;
        if (measureQ && chords.length) {
          const target = Math.round(measureQ * DIVISIONS);
          ...
          if (padDur > 0) {
            ...
            for (let k = 0; k <= chords.length; k++) {
              ...
              for (let i = 0; i < chords.length; i++) {
                const at = t + (i >= k ? padDur : 0);
                if (chords[i].bx !== undefined)
                  cost += Math.abs(at / DIVISIONS - chords[i].bx * measureQ);
                t += durs[i];
              }
              if (cost < best - 1e-9) { best = cost; padAt = k; }
            }
          }
        }
        chords.forEach((c, ci) => {
          if (padDur > 0 && ci === padAt) xml += restXML(padDur / DIVISIONS, padDur);
        ...
        if (padDur > 0 && padAt === chords.length) xml += restXML(padDur / DIVISIONS, padDur);
        if (measureQ && !chords.length) {
          xml += restXML(measureQ, Math.round(measureQ * DIVISIONS));
        } else if (!measureQ && !chords.length) {
          xml += restXML(1);
        }
        xml += `</measure>`;
      }
