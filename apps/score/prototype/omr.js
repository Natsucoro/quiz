...
(function (global) {
  ...
  function findRests(bin, vRunLen, W, H, staves, heads) {
    ...
    for (let si = 0; si < staves.length; si++) {
      ...
      while (i < col.length) {
        ...
        i = j + 1;
        if (w < S * 0.40 || w > S * 1.75) continue;
        // 幅が符頭1個ぶんを超える区間は、休符に偽の符頭が重なって
        // 膨らんでいる場合だけ休符として扱う（下の取り消し判定で決める）
        const wideSeg = w > S * 1.45;
        ...
        // にじみで3.7線間を超えて写ることがあり、その分（〜4.0）は
        // 下の「偽符頭の取り消し」が成立した場合だけ休符と認める
        if (h >= S * 2.1 && h <= S * 4.0 && w >= S * 0.70) q = 1;
        else if (h >= S * 1.1 && h < S * 2.1 && w >= S * 0.55) q = 0.5;
        if (!q) continue;
        // 符頭がすぐ近くにあるものは、臨時記号か符頭そのもの……のことが多いが、
        // 休符の塊そのものの上に符頭が生まれていることがある（4分休符のジグザグや
        // 8分休符の頭は符頭と同じ密度で、偽の音符＋偽の32分になっていた）。
        // 近くの符頭が「全部この塊の内側」で、塊が符頭の和音にしては
        // すかすか・縦線も短いなら、休符のほうを信じて符頭を取り消す
        const conflict = mine.filter(n => n.x > x0 - S * 0.6 && n.x < x1 + S * 2.0);
        if (conflict.length) {
          const inside = conflict.filter(n => !n.hollow &&
            n.x >= x0 - S * 0.2 && n.x <= x1 + S * 0.2 &&
            n.y >= ya - S * 0.3 && n.y <= yb + S * 0.3);
          if (inside.length !== conflict.length) continue;   // 本物の符頭が近い
          // 休符の腕を梁と数えた符頭は梁2本以上（16分・32分）になる。
          // 本物の和音（4分・8分、梁0〜1本）はこの条件で守られる
          if (!inside.every(n => n.split || (n.beams || 0) >= 2)) continue;
          let ink = 0;
          for (let x = x0; x <= x1; x++)
            for (let y = ya; y <= yb; y++) if (bin[y * W + x]) ink++;
          const fill = ink / (w * h);
          if (fill < 0.25 || fill > 0.70 || vmax > S * 3.35) continue;  // 密なら和音
          for (const n of inside) n._restKill = true;
        } else if (wideSeg || h > S * 3.7) continue;   // 広い・高すぎる区間は取り消しの根拠が無ければ休符にしない
  ...
  function findAccidentalGlyphs(bin, runLen, W, H, staves) {
    ...
    for (let si = 0; si < staves.length; si++) {
      ...
      /** x列 [i..j]・行 [yLo..yHi] の範囲を記号として採点する */
      const judge = (iSeg, jSeg, yLo, yHi) => {
        const w = jSeg - iSeg + 1;
        if (w < S * 0.42 || w > S * 1.05) return null;
        let top = 1e9, bot = -1, vmax = 0, ink = 0, beamy = 0;
        const colV = [];
        for (let ci = iSeg; ci <= jSeg; ci++) {
          const x = xa + ci;
          let cInk = 0, cTop = -1, cBot = -1, run = 0, cV = 0;
          for (let y = yLo; y <= yHi; y++) {
            if (bin[y * W + x]) {
              cInk++; if (cTop < 0) cTop = y; cBot = y;
              if (++run > cV) cV = run;
              if (runLen[y * W + x] > S * 2.2) beamy++;
            } else run = 0;
          }
          colV.push(cV);
          if (cTop >= 0 && cTop < top) top = cTop;
          if (cBot > bot) bot = cBot;
          if (cV > vmax) vmax = cV;
          ink += cInk;
        }
        if (bot < 0) return null;
        const hgt = bot - top + 1;
        if (hgt < S * 1.45 || hgt > S * 3.4) return null;
        if (vmax < S * 1.15 || vmax > S * 3.2) return null;
        if (ink && beamy / ink > 0.30) return null;
        const fill = ink / (w * hgt);
        if (fill < 0.18 || fill > 0.62) return null;
        const cy = (top + bot) / 2;
        if (cy < st.top - S * 1.2 || cy > st.bottom + S * 1.2) return null;
        ...
        return { x0: xa + iSeg, x1: gx1, y0: top, y1: bot,
                 x: (xa + iSeg + gx1) / 2, y: cy, staff: si, acc };
      };
      ...
      while (i < cols.length) {
        ...
        const iSeg = i, jSeg = j;
        i = j + 1;
        const g = judge(iSeg, jSeg, y0, y1);
        if (g) { out.push(g); continue; }
        // 和音の前では記号どうしが縦に重なって並ぶ（D と B に♮、など）。
        // ひとつぶんの高さを超える塊は、黒のいちばん薄い行で上下に割って
        // それぞれをもう一度採点する
        let top = 1e9, bot = -1;
        for (let ci = iSeg; ci <= jSeg; ci++) {
          const c = cols[ci];
        ...
        if (bot < 0) continue;
        const hgt = bot - top + 1;
        if (hgt <= S * 3.4 || hgt > S * 6.2) continue;
        let cut = -1, cutInk = 1e9;
        for (let y = Math.round(top + S * 1.2); y <= Math.round(bot - S * 1.2); y++) {
          let rowInk = 0;
          for (let ci = iSeg; ci <= jSeg; ci++) if (bin[y * W + xa + ci]) rowInk++;
          if (rowInk < cutInk) { cutInk = rowInk; cut = y; }
        }
        if (cut < 0) continue;
        const gA = judge(iSeg, jSeg, top, cut - 1);
        const gB = judge(iSeg, jSeg, cut + 1, bot);
        if (gA) out.push(gA);
        if (gB) out.push(gB);
      }
    }
  ...
  function findAccidental(bin, vRunLen, runLen, W, H, st, h, heads, stem) {
    ...
    let xa = Math.max(Math.round((st.xMusic || 0)) - 2, Math.round(h.x - S * 1.55));
    const xb = Math.round(h.x - S * 0.62);
    // 行の途中の音部記号の胴は♭そっくり。記号変更の直後の音に
    // 偽の♭が付いて半音下がる（実測でF3の和音がE3に化けた）
    if (st.midClefs) for (const mc of st.midClefs)
      if (mc.x1 + S * 0.4 > xa && mc.x0 < xb) xa = Math.max(xa, Math.round(mc.x1 + S * 0.4));
  ...
  function hasDot(ii, W1, W, H, n, S) {
    const r = Math.max(1, Math.round(S * 0.14));      // 付点そのもの
    const area = (r * 2 + 1) * (r * 2 + 1);
    // 孤立の検査は上・右・下だけ見る。左は符頭そのものがあるのが正常なので、
    // 左まで見ると付点が全部落ちる（実測で、付点だらけの楽譜で検出0件だった）。
    // 検査の帯は点の縁から1画素あけて置く。点自身の縁を数えると必ず落ちる
    const m1 = r + 1, m2 = r + 3;
    for (let dx = Math.round(S * 0.78); dx <= Math.round(S * 1.55); dx++) {
      for (let dy = -Math.round(S * 0.65); dy <= Math.round(S * 0.65); dy++) {
        // 付点は必ず「間」に打たれる。符頭から見て 0（同じ間）か ±半線間の
        // 近くにしか来ない。それ以外の高さで見つかる黒は付点ではない
        const g = Math.abs(dy) / S;
        if (Math.min(g, Math.abs(g - 0.5)) > 0.17) continue;
        const x = Math.round(n.x + dx), y = Math.round(n.y + dy);
        if (x - r < 0 || x + m2 >= W || y - m2 < 0 || y + m2 >= H) continue;
        const inner = rectSum(ii, W1, x - r, y - r, x + r, y + r);
        if (inner / area < 0.78) continue;
        const right = rectSum(ii, W1, x + m1, y - r, x + m2, y + r) /
                      ((m2 - m1 + 1) * (r * 2 + 1));
        const top = rectSum(ii, W1, x - r, y - m2, x + r, y - m1) /
                    ((r * 2 + 1) * (m2 - m1 + 1));
        const bot = rectSum(ii, W1, x - r, y + m1, x + r, y + m2) /
                    ((r * 2 + 1) * (m2 - m1 + 1));
        if (right < 0.20 && top < 0.20 && bot < 0.20) return true;
      }
    }
    return false;
  }
  ...
  function findMidClefs(bin, W, H, st, runLen) {
    ...
    while (i < colInk.length) {
      ...
      const seg = { i, j }; i = j + 1;
      // 行の途中のヘ音記号。胴は五線の上半分に収まり（符尾が無いので
      // 下線まで届かない）、すぐ右の1つ目・2つ目の間に点が2つ並ぶ。
      // 左手がト音で始まりヘ音に戻る楽譜で、これを見ないと以降の
      // 左手が2オクターブ近く上に化ける
      if (w >= S * 1.2 && w <= S * 2.8 &&
          top >= st.top - S * 0.9 && top <= st.top + S * 0.5 &&
          bot <= st.bottom - S * 0.25 &&
          bot - top >= S * 2.0 && bot - top <= S * 3.7) {
        const dotAt = stepFromTop => {
          const cy = Math.round(st.top + stepFromTop * S);
          const xa = x0 + seg.j + Math.max(1, Math.round(S * 0.1));
          const xb = Math.min(W - 1, x0 + seg.j + Math.round(S * 0.9));
          const dv = Math.round(S * 0.3);
          for (let x = xa; x <= xb; x++)
            for (let y = Math.max(0, cy - dv); y <= Math.min(H - 1, cy + dv); y++)
              if (bin[y * W + x]) return true;
          return false;
        };
        if (dotAt(0.5) && dotAt(1.5)) {
          out.push({ x: x0 + (seg.i + seg.j) / 2, x0: x0 + seg.i,
                     x1: x0 + seg.j, clef: "F" });
          continue;
        }
      }
  ...
  function readScore(img, opts) {
    ...
    // 符頭に癒着した臨時記号。記号と符頭の間に白い隙間が無いと、
    // 前処理の記号消しでは切り分けられず、検出の中心が記号側に引っ張られて
    // 音の高さが半段ずれる（実測でE♮がFに化けていた）。
    // 「符頭の左上と左下の両方に、符頭の高さを越えてはみ出す黒」が癒着の印。
    // 見つけたら中心を右（符頭側）へ寄せ、記号は♮として効かせる。
    for (const h of heads) {
      if (h.hollow || h.split) continue;
      const st = staves[h.staff];
      const S = st.space;
      // 五線の外の音は見ない。加線や梁が「はみ出し」に見えて誤爆する
      if (h.y < st.top - S * 0.5 || h.y > st.bottom + S * 0.5) continue;
      ...
      const xl = h.x - S * 1.30, xr = h.x - S * 0.62;
      const ul = box(xl, h.y - S * 1.55, xr, h.y - S * 0.75);
      const ll = box(xl, h.y + S * 0.75, xr, h.y + S * 1.55);
      const need = S * S * 0.16;
      if (ul < need || ll < need) continue;
      // 上下のはみ出しが横に長いなら梁。細い縦棒（記号のはみ出し）だけを認める
      const wideUl = box(h.x - S * 2.3, h.y - S * 1.55, xl - 1, h.y - S * 0.75);
      const wideLl = box(h.x - S * 2.3, h.y + S * 0.75, xl - 1, h.y + S * 1.55);
      if (wideUl > need * 1.2 || wideLl > need * 1.2) continue;
      // 高さの測り直しにだけ、記号ぶん右に寄せた x を使う。
      // 音符の位置そのものを動かすと、小節や和音の割り付けが崩れる
      h.xc = h.x + S * 0.30;
      h.accFused = 0;           // ♮とみなす（癒着するのはほぼ♮）
    }
    ...
    for (const h of heads) {
      ...
      const cx0 = Math.round(h.xc !== undefined ? h.xc : h.x);
      let sum = 0, n = 0;
      for (let x = cx0 - rx; x <= cx0 + rx; x++) {
        if (x < 0 || x >= W) continue;
    ...
    heads = heads.filter(h => {
      ...
      if (Math.abs(raw - Math.round(raw)) <= 0.34) return true;
      // 癒着した記号を切り離した音は、測り直しがぶれやすい。
      // 落とすと小節の中身が変わって後ろ全部がずれるので、最寄りの段に吸着させる
      if (h.accFused !== undefined) {
        h.y = baseY - Math.round(raw) * (st.space / 2);
        return true;
      }
      return false;
    });
    ...
    // （実測でジョプリンの左手の4音和音が、1個の音としか読まれていなかった）
    {
      ...
      for (const h of heads) {
        ...
        // 等分割だと中の音が半段ずれる。いちばん上の音を段の格子に吸着させ、
        // そこから正確に1線間（3度）ずつ下へ置く
        const baseY = st.bottom + (st.slope || 0) * (h.x - W / 2);
        const topRaw = (baseY - (ya + S * 0.5)) / (S / 2);
        const topY = baseY - Math.round(topRaw) * (S / 2);
        for (let i = 0; i < k; i++) {
          const y = topY + i * S;
          if (y > yb + S * 0.3) continue;
          ...
          // 置こうとする位置に符頭ぶんの黒が実際にあることを確かめる。
          // 縦の走査は1行おきの隙間を許すので、白地の上まで歩いて
          // 幻の音符を置くことがある（実測でほぼ白地に3音の偽和音が出た）
          const bw = Math.round(S * 0.45), bh = Math.round(S * 0.32);
          const xx = Math.round(h.x), yy = Math.round(y);
          if (xx - bw < 0 || xx + bw >= W || yy - bh < 0 || yy + bh >= H) continue;
          const fill = rectSum(ii, W + 1, xx - bw, yy - bh, xx + bw, yy + bh) /
                       ((bw * 2 + 1) * (bh * 2 + 1));
    ...
    // 五線の外の「白抜きの符頭」が単語の中の1文字でないことを確かめる。
    // 発想標語（Allegro 等）の e・g・o は白抜きの符頭そっくりで、
    // 加線の高さより五線に近いと上の検査をすり抜ける。文字は単語として
    // 並ぶので、同じ高さの左右両方に別の文字のインクが見えたら棄てる。
    // 塗りつぶしの符頭は文字と紛れないので対象にしない（16分の連なりを守る）
    heads = heads.filter(h => {
      if (!h.hollow) return true;
      const st = staves[h.staff];
      const S = st.space;
      const dist = h.y < st.top ? st.top - h.y : (h.y > st.bottom ? h.y - st.bottom : 0);
      if (dist <= S * 0.35) return true;
      const band = side => {
        const xa = Math.round(h.x + (side < 0 ? -S * 2.4 : S * 1.1));
        const xb = Math.round(h.x + (side < 0 ? -S * 1.1 : S * 2.4));
        const ya = Math.max(0, Math.round(h.y - S * 0.25));
        const yb = Math.min(H - 1, Math.round(h.y + S * 0.25));
        let ink = 0, n = 0;
        for (let y = ya; y <= yb; y++)
          for (let x = Math.max(0, xa); x <= Math.min(W - 1, xb); x++) { n++; if (cleaned[y * W + x]) ink++; }
        return n ? ink / n : 0;
      };
      return !(band(-1) >= 0.22 && band(1) >= 0.22);
    });
    ...
    // 記号は「その音から小節の終わりまで、同じ高さの音」に効く。
    {
      ...
      for (const n of sorted) {
        ...
        // 癒着から推定した♮は確度が低いので、その音にだけ効かせて伝播させない
        if (n.acc !== undefined && !n.accOnly) live.set(k, n.acc);
        let a = live.get(k);
        if (n.accOnly && n.acc !== undefined) a = n.acc;
    ...
    for (let i = 0; i < linked.length; ) {
      ...
      if (group.length > 1) {
        // 梁の角や付け根が「反対向きの符尾の音符」として拾われることがある
        // （実測で16分の連なりの上に偽のソが5個、どれも0.25拍ずつ後ろへずらしていた）。
        // 群れの符尾の向きの多数決に反し、符頭の位置を梁そのもの（長い斜めの帯）が
        // 貫いているものは、梁を符頭と見ているだけなので棄てる。
        // 2声部の連桁で向きが割れても、本物の符頭は梁に貫かれないので巻き添えにならない
        const dirSum = group.reduce((a, g) => a + (g.stemDir || 0), 0);
        if (dirSum !== 0) {
          const maj = dirSum > 0 ? 1 : -1;
          const majY = group.filter(g => g.stemDir === maj).map(g => g.y);
          const S2 = staves[group[0].staff].space;
          const lo = Math.min.apply(null, majY), hi2 = Math.max.apply(null, majY);
          const onBeam = g => {
            const cx = Math.round(g.x), cy = Math.round(g.y);
            for (let s = -0.45; s <= 0.46; s += 0.15) {
              let len = 1;
              for (const dir of [-1, 1]) {
                for (let k = 1; k < S2 * 3.5; k++) {
                  const xx = cx + dir * k, yy = Math.round(cy + s * dir * k);
                  if (xx < 0 || xx >= W || yy < 1 || yy >= H - 1) break;
                  if (!cleaned[yy * W + xx] && !cleaned[(yy - 1) * W + xx] &&
                      !cleaned[(yy + 1) * W + xx]) break;
                  len++;
                }
              }
    ...
    // 同じ符尾の柱を「上向き」と「下向き」で取り合う2音は、片方が
    // 旗の巻き尻や符尾の付け根を符頭と見た偽物（実測で8分の旗の先が
    // 1音上の音符に化けていた）。符頭の窓の黒の詰まりが薄いほうを棄てる
    {
      const W1f = W + 1;
      const headFill = n => {
        const S = n.space;
        const bw = Math.round(S * 0.42), bh = Math.round(S * 0.30);
        const x = Math.round(n.x), y = Math.round(n.y);
        if (x - bw < 0 || x + bw >= W || y - bh < 0 || y + bh >= H) return 1;
        return rectSum(ii, W1f, x - bw, y - bh, x + bw, y + bh) /
               ((bw * 2 + 1) * (bh * 2 + 1));
      };
      // 白抜きの符頭は「白い芯を黒い輪が囲む」形をしているかで本物かを見る
      const ringOK = n => {
        const S = n.space;
        const x = Math.round(n.x), y = Math.round(n.y);
        const ibx = Math.max(1, Math.round(S * 0.22)), iby = Math.max(1, Math.round(S * 0.18));
        if (x - S < 1 || x + S >= W - 1 || y - S < 1 || y + S >= H - 1) return false;
        const inner = rectSum(ii, W1f, x - ibx, y - iby, x + ibx, y + iby) /
                      ((ibx * 2 + 1) * (iby * 2 + 1));
        if (inner > 0.55) return false;
        let dark = 0;
        for (let a2 = 0; a2 < 12; a2++) {
          const th = a2 * Math.PI / 6;
          const px = Math.round(x + Math.cos(th) * S * 0.50);
      ...
      for (let a = 0; a < notes.length; a++) {
        for (let b = a + 1; b < notes.length; b++) {
          ...
          if (na.staff !== nb.staff) continue;
          if (!na.stemDir || !nb.stemDir || na.stemDir === nb.stemDir) continue;
          if (Math.abs(na.stemX - nb.stemX) > 2) continue;
          if (Math.abs(na.y - nb.y) < na.space * 1.5) continue;
          if (na.hollow || nb.hollow) {
          ...
          if (fa < 0.68 && fb >= 0.68) kill.add(na);
          else if (fb < 0.68 && fa >= 0.68) kill.add(nb);
        }
      }
      if (kill.size) notes = notes.filter(n => !kill.has(n));
    }

    // 梁を符頭と見ていた偽の音を外す。ただし多くの場合、その位置には
    // 読めていない何か（休符・付点ぶんの間）が本当にあるので、
    // 単独で立っていた偽音は同じ長さの休符に置き換えて時間を保つ
    const ghostRests = [];
    for (const n of notes) {
      if (!n._beamGhost) continue;
      const alone = !notes.some(o => o !== n && !o._beamGhost &&
        o.staff === n.staff && Math.abs(o.x - n.x) <= n.space * 0.6);
      if (alone) ghostRests.push({ x: n.x, staff: n.staff, q: n.q });
    }
    notes = notes.filter(n => !n._beamGhost);

    // 休符も拾う。読めないと、そこから後ろの音が全部前に詰まる
    const rests = findRests(cleaned, vRunLen, W, H, staves, notes);
    rests.push(...ghostRests);
    // 休符の塊の上に生まれていた偽の符頭は、休符を採って取り消す
    notes = notes.filter(n => !n._restKill);
